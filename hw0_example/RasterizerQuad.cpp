...
void RasterizerQuad::rasterize(glm::mat4 M, glm::mat4 V, glm::mat4 P, glm::mat4 D, PointCloud& obj) {
	 ...
	 */
    std::vector<glm::vec3> obj_points = obj.getPoints();
    std::vector<glm::vec3> obj_vec_normal = obj.getVec_normal();
    GLfloat pointsize = obj.getPointsize()-1;
    if(pointsize<1){
        pointsize = 1;
    }
    for(int i = 0; i<obj_points.size();i++){
        glm::vec3 p = obj_points[i];
        glm::vec3 color_element = 0.5f*normalize(obj_vec_normal[i])+0.5f;
        float r = color_element[0];
        float g = color_element[1];
        float b = color_element[2];
        glm::vec4 p_ = D*P*V*M*vec4(p,1.0f);
        for(int x = 0; x<pointsize;x++){
            for(int y = 0; y<pointsize;y++){
                drawPoint(p_[0]/p_[3]+x,p_[1]/p_[3]+y,p_[2]/p_[3],r,g,b);
            }
        }
        //drawPoint(p_[0]/p_[3],p_[1]/p_[3],p_[2]/p_[3],color_element[0],color_element[1],color_element[2]);
    }
